...
>
> `ani-scale-lanczos.py`에는 리사이즈 후 알파 채널을 정리하는 `_cleanup_alpha(result, alpha_low=50, alpha_high=248)` 함수가 있습니다. 원본 커서는 대부분 binary alpha(0 또는 255)이므로, Lanczos 보간으로 생긴 극단적 반투명 값을 다음과 같이 정리합니다:
>
...
> | 조절 | 효과 |
> |------|------|
> | `alpha_low` ↑ (예: 50→80) | 유령 픽셀이 더 공격적으로 제거되지만, 안티앨리어싱 가장자리가 거칠어질 수 있음 |
> | `alpha_low` ↓ (예: 50→15) | 더 부드러운 가장자리를 유지하지만, 유령 픽셀이 남을 수 있음 |
> | `alpha_high` ↓ (예: 248→220) | 반투명 가장자리가 줄어들어 더 선명해지지만 거칠어질 수 있음 |
> | `alpha_high` ↑ (예: 248→253) | 더 부드러운 불투명 전환, 가장자리에 약간의 반투명 띠가 남을 수 있음 |
