...
리사이즈 없이 원본 크기 그대로 변환하려면:

```bash
./make-cursor-default-size.sh <ThemeName>
```

예시:

```bash
./make-cursor.sh Klukai            # 기본값: lanczos (32→48 리사이즈)
./make-cursor.sh Klukai nearest    # nearest neighbor 사용
./make-cursor-default-size.sh Klukai  # 원본 크기 유지
```
...
>
> `ani-scale-lanczos.py`에는 리사이즈 후 알파 채널을 정리하는 `_cleanup_alpha(result, alpha_low=64, alpha_high=255)` 함수가 있습니다. 원본 커서는 대부분 binary alpha(0 또는 255)이므로, Lanczos 보간으로 생긴 극단적 반투명 값을 다음과 같이 정리합니다:
>
> - `alpha < alpha_low` → 0으로 변환 (유령 픽셀 제거)
> - `alpha > alpha_high` → 255로 변환 (거의 불투명한 픽셀 확정)
> - 중간값은 유지 (자연스러운 안티앨리어싱 가장자리)
>
> 기본값 `alpha_high=255`는 high 클램핑을 비활성화하여 안티앨리어싱된 반투명 가장자리를 최대한 보존합니다. `alpha_low=64`는 유령 픽셀을 제거하면서도 부드러운 가장자리를 유지하는 균형점입니다.
>
> 이 값을 조절하면 가장자리 렌더링을 커서 테마에 맞게 미세 조정할 수 있습니다:
>
> | 조절 | 효과 |
> |------|------|
> | `alpha_low` ↑ (예: 64→120) | 유령 픽셀이 더 공격적으로 제거되지만, 안티앨리어싱 가장자리가 거칠어질 수 있음 |
> | `alpha_low` ↓ (예: 64→30) | 더 부드러운 가장자리를 유지하지만, 유령 픽셀이 남을 수 있음 |
> | `alpha_high` ↓ (예: 255→170) | 거의 불투명한 픽셀을 완전 불투명으로 확정하여 선명해지지만 거칠어질 수 있음 |
